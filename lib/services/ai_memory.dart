...
class AiMemory {
  ...
  static Future<List<Map<String, dynamic>>> getUserHistory({
    ...
    try {
      ...
      final response = await get('/ai-chats?userType=$userType&userId=$userId&limit=$limit');
  ...
  static Future<List<Map<String, dynamic>>> getDailyMessages({
    ...
    try {
      ...
      final response = await get('/ai-chats?userType=$userType&startDate=${startDate.toIso8601String()}');
      if (response != null && response['data'] != null) {
        return List<Map<String, dynamic>>.from(response['data']);
      }
