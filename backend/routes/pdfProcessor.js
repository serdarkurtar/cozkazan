...
// Test oluşturma
async function createTestFromQuestions(questions, testName, sinifId, dersId, konuId) {
    const tests = [];
    
    // Önce havuzu bul veya oluştur
    ...
    if (!havuz) {
      havuz = new TestHavuzu({ sinif: sinifId, ders: dersId, konu: konuId, testler: [], createdAt: new Date() });
      await havuz.save();
    }
    ...
    questions.forEach((question, index) => {
        if (question.options.length >= 2) { // En az 2 şık olmalı
            const test = {
                ...
                sinif: sinifId, // sinifId yerine sinif
                ders: dersId,   // dersId yerine ders
                konu: konuId,   // konuId yerine konu
                havuzId: havuz._id, // HAVUZ ID EKLENDİ!
                sorular: [{
...
router.post('/upload-pdf', upload.single('pdf'), async (req, res) => {
    try {
        ...
        // Testleri oluştur ve veritabanına kaydet
        const tests = await createTestFromQuestions(questions, testName, sinifId, dersId, konuId);
        ...
        for (const testData of tests) {
            ...
            // Havuza testi ekle
            const havuz = await TestHavuzu.findOne({ sinif: sinifId, ders: dersId, konu: konuId });
            if (havuz) {
        ...
        res.json({
            success: true,
            message: `${savedTests.length} test başarıyla oluşturuldu ve havuza eklendi`,
            tests: savedTests,
            totalQuestions: questions.length,
            processedQuestions: savedTests.length,
