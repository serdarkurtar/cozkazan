      ...
      if (!test.sinifAdi && test.sinif) {
        try {
          ...
          if (sinif) {
            ...
            // Eğer sinif bulunamazsa, sinif alanındaki değeri kullan
            guncelleme.sinifAdi = test.sinif;
            guncellemeGerekli = true;
            console.log(`✅ Test ${test.testAdi} için sinifAdi: ${test.sinif} (ID kullanıldı)`);
          }
        } catch (err) {
          // Hata durumunda sinif alanındaki değeri kullan
          guncelleme.sinifAdi = test.sinif;
          guncellemeGerekli = true;
          console.log(`✅ Test ${test.testAdi} için sinifAdi: ${test.sinif} (ID kullanıldı)`);
        }
      ...
      if (!test.dersAdi && test.ders) {
        try {
          ...
          if (ders) {
            ...
            // Eğer ders bulunamazsa, ders alanındaki değeri kullan
            guncelleme.dersAdi = test.ders;
            guncellemeGerekli = true;
            console.log(`✅ Test ${test.testAdi} için dersAdi: ${test.ders} (ID kullanıldı)`);
          }
        } catch (err) {
          // Hata durumunda ders alanındaki değeri kullan
          guncelleme.dersAdi = test.ders;
          guncellemeGerekli = true;
          console.log(`✅ Test ${test.testAdi} için dersAdi: ${test.ders} (ID kullanıldı)`);
        }
      ...
      if (!test.konuAdi && test.konu) {
        try {
          ...
          if (konu) {
            ...
            // Eğer konu bulunamazsa, konu alanındaki değeri kullan
            guncelleme.konuAdi = test.konu;
            guncellemeGerekli = true;
            console.log(`✅ Test ${test.testAdi} için konuAdi: ${test.konu} (ID kullanıldı)`);
          }
        } catch (err) {
          // Hata durumunda konu alanındaki değeri kullan
          guncelleme.konuAdi = test.konu;
          guncellemeGerekli = true;
          console.log(`✅ Test ${test.testAdi} için konuAdi: ${test.konu} (ID kullanıldı)`);
        }
