...
export async function getOrCreateVarsayilanHavuz(sinifId, dersId, konuId) {
    try {
        // ÖNCE MEVCUT HAVUZU KONTROL ET
        const mevcutHavuz = await TestHavuzu.findOne({ 
            sinif: sinifId, 
            ders: dersId, 
            konu: konuId 
        });
        ...
        if (mevcutHavuz) {
            console.log(`[HAVUZ] Mevcut havuz bulundu: ${mevcutHavuz._id}`);
            return mevcutHavuz;
        }
        
        // KonuId string ise, önce konuyu adına göre bul
        let konu;
        if (mongoose.Types.ObjectId.isValid(konuId)) {
            konu = await Konu.findById(konuId);
        } else {
            // String ise konu adına göre bul
            konu = await Konu.findOne({ ad: konuId });
        }
        
        if (!konu) {
            // Konu yoksa, direkt havuz oluştur (konu adını kullan)
            const yeniHavuz = new TestHavuzu({
                sinif: sinifId,
                ders: dersId,
                konu: konuId, // String konu adını kullan
                testler: [],
                havuzAdi: `${konuId} - Ana Havuz`,
                havuzTipi: 'varsayilan'
            });

            await yeniHavuz.save();
            console.log(`[HAVUZ] Yeni havuz oluşturuldu (konu bulunamadı): ${yeniHavuz._id}`);
            return yeniHavuz;
        }
        ...
        // Konuyu güncelle (eğer konu varsa)
        if (konu._id) {
