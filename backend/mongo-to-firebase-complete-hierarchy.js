...
async function migrateMongoToFirebaseComplete() {
  try {
    console.log('🔄 MongoDB bağlantısı kuruluyor...');
    await mongoose.connect(mongoUrl);
    console.log('✅ MongoDB bağlantısı başarılı');

    console.log('🔥 Firebase\'e tam hiyerarşi ile veri aktarımı başlıyor...');
    ...
    console.log(`✅ ${konular.length} konu aktarıldı (TÜM KONULAR!)`);
    ...
    for (const test of testler) {
      ...
      await addDoc(collection(db, 'testler'), {
        ...
        sinifAdi: test.sinifAdi || '2. Sınıf', // Varsayılan değer
        dersAdi: test.dersAdi || 'Türkçe', // Varsayılan değer
        konuAdi: test.konuAdi || 'Konu', // Varsayılan değer
    ...
    for (const havuz of testHavuzlari) {
      ...
      await addDoc(collection(db, 'test_havuzlari'), {
        ...
        sinifAdi: havuz.sinifAdi || '2. Sınıf', // Varsayılan değer
        dersAdi: havuz.dersAdi || 'Türkçe', // Varsayılan değer
        konuAdi: havuz.konuAdi || 'Konu', // Varsayılan değer
    ...
    console.log('🎉 TÜM VERİLER BAŞARIYLA FIREBASE\'E AKTARILDI!');
    console.log(`📊 Aktarılan veriler:`);
    console.log(`   - ${siniflar.length} sınıf`);
    console.log(`   - ${dersler.length} ders`);
    console.log(`   - ${konular.length} konu (TÜM 111 KONU!)`);
    ...
    console.log('   Her dersin kendi konuları var');
    console.log('   Her konunun kendi testleri var');
    console.log('   Tüm 111 konu doğru hiyerarşi ile aktarıldı!');
...
migrateMongoToFirebaseComplete(); 